<?php 
	...
	//$result = mysqli_query($link,"SELECT * FROM LeaveAvailable where OfficeID = '$OfficeID' and Date = '$Date'");
	
	$sql = 'SELECT OfficeID, convert(varchar, Date, 120) as Date, LeaveAvailable FROM dbo.LeaveAvailable 
	where OfficeID = (?) and Date = (?)';
	
	$param = array($OfficeID,$Date);
